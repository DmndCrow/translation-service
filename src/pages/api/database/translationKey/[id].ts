...
export default async function handler(
  ...
  res: NextApiResponse
    ...
    const createTranslationValue = async (keyId: number, languageId: number, value: string) => {
      return await prisma.translationValue.upsert({
        where: { languageId_translationKeyId: { translationKeyId: keyId, languageId: languageId } },
        update: { value: value },
